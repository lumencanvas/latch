...
const showVisibility = computed(() => (props.data?.showVisibility as boolean) ?? true)
...
/**
 * Draw connections with visibility-based alpha
 */
function drawPoseConnections(
  ...
  const { color = '#00ff00', lineWidth: lw = 2 } = options
  ...
  for (const [startIdx, endIdx] of POSE_CONNECTIONS) {
    ...
    // Skip if both points have very low visibility
    if (startVis < 0.3 && endVis < 0.3) continue

    // Use average visibility for line alpha if showVisibility enabled
    const alpha = showVisibility.value
      ? Math.min(1, (startVis + endVis) / 2 + 0.3)
      : 1

    c.globalAlpha = alpha
    c.beginPath()
    c.moveTo(start.x * width, start.y * height)
    c.lineTo(end.x * width, end.y * height)
    c.stroke()
  }
...
/**
 * Draw landmarks with visibility-based alpha
 */
function drawPoseLandmarks(
  ...
  const { color = '#00ff00', pointSize: ps = 4 } = options
  ...
  for (const lm of landmarks) {
    ...
    const alpha = showVisibility.value ? Math.min(1, visibility + 0.3) : 1
    c.globalAlpha = alpha
...
function updateDisplay() {
  ...
  if (loading) {
    ...
    drawLabel(c, 'No pose', 8, 18, { color: '#ff6666' })
  }
  ...
  if (showOverlay.value && detected && landmarks && landmarks.length > 0) {
    ...
    if (mode === 'skeleton') {
      // Draw skeleton connections with visibility
      drawPoseConnections(c, landmarks, width, height, drawOpts)
      // Draw landmark points with visibility
      drawPoseLandmarks(c, landmarks, width, height, drawOpts)
    } else if (mode === 'points') {
      // Draw landmark points only
      drawLandmarks(c, landmarks, width, height, drawOpts)
    } else if (mode === 'bbox') {
    ...
    // Draw info label
    drawLabel(c, 'POSE', 8, height - 8, { color: overlayColor.value })
  }
  ...
  >
    ...
    <div class="node-content">
      ...
      >
        ...
        />
        <span class="node-title">Pose</span>
      </div>
    ...
    <div class="handles-column handles-right">
      ...
      <div class="handle-slot">
        <Handle
          ...
          class="port-handle"
        />
        <span class="port-label port-label-right">Nose</span>
      </div>
      <div class="handle-slot">
        <Handle
          id="leftWrist"
          ...
          class="port-handle"
        />
        <span class="port-label port-label-right">L Wrist</span>
      </div>
      <div class="handle-slot">
        <Handle
          id="rightWrist"
          ...
          class="port-handle"
        />
        <span class="port-label port-label-right">R Wrist</span>
      </div>
      <div class="handle-slot">
        <Handle
          id="detected"
