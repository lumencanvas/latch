...
- [~] `mod/p5-touch-connect` — **Partial:** `connectionMode: Loose` set on the editor `<VueFlow>` (`isValidConnection` still enforces type validity) and handle hit-targets enlarged under `@media (pointer: coarse)`. **Remaining:** verify/enable explicit tap-source→tap-target (`connect-on-click`); the bigger gap is that the node **palette is drag-only (HTML5 DnD), so nodes can't be placed by touch** — needs a tap-to-add path (insert at viewport center / last-tapped point). Component test for tap-to-connect still to write.
